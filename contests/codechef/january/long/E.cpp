...
ll dp[N][2];
ll knapSack(int W1, int W2, int i, int pos) 
{ 
  debug(W1, W2, i, pos);
  if ( W1 <= 0 && W2 <= 0) {
    return 0;
  ...
  if (W1 >= 0)
    dp[i][pos] = min({dp[i][pos],
      knapSack(W1 - h[i], W2, i + 1, 0),
    }) + 1;
  if (W2 >= 0)
    dp[i][pos] = min({dp[i][pos],
        knapSack(W1, W2 - h[i], i + 1, 1)
    }) + 1;
  return dp[i][pos];
}
void solve() {
  ...
  range(i, n) range(j,2) dp[i][j] = INF;
  sort(h, h + n, greater<ll>());
  // reverse(h, h + n);
  ll ans = knapSack(k, k, 0, 0);
  if (ans >= INF) {
    cout << -1 << '\n';
  } 
  else 
