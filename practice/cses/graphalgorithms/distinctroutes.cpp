...
bool bfs(ll start, ll end) {
  ...
  while(q.size()) {
    ...
    q.pop();
    range(i, n) {
      debug(u, i, graph[u][i], edges[graph[u][i]]);
      if (!visited[i] && graph[u][i] >= 0 && edges[graph[u][i]]) {
        debug(u, i, graph[u][i], edges[graph[u][i]]);
        parent[i] = u;
        visited[i] = 1;
        q.push(i);
...
void dfs2(ll u=0) {
  ans.push_back(u);
  range(i, n) {
    if(graph[u][i]%2==0 && !edges[graph[u][i]]) {
      dfs2(i);
      edges[graph[u][i]] = 1;
      return;
    }
...
void ford_fulkerson()
{
    ...
    while(bfs(0, n-1)) {
      ...
      for(ll v= n-1 ; v!= 0; v=parent[v]) {
        ll u = parent[v];
        if (path_flow > edges[graph[u][v]]) {
          path_flow = edges[graph[u][v]];
        }
      }
      for(ll v= n-1 ; v!= 0; v=parent[v]) {
        ll u = parent[v];
        edges[graph[u][v]] -= path_flow;
        edges[graph[v][u]] += path_flow;    
      }
      max_flow += path_flow;
    }
    cout << max_flow << '\n';
...
void init() {
  ...
  range(i, m) {
    ...
    if (graph[b][a] == -1)
      graph[b][a] = 2*i + 1;
    edges[graph[a][b]] += 1;
    // debug(a, b, graph[a][b], graph[b][a], edges[graph[a][b]]);
  }
  ford_fulkerson();
