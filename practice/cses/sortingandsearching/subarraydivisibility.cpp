...
void init() { 
  ...
  map <ll, vector <int>> m;
  range(i,n) {
    prefix_sum = (prefix_sum + a[i] + abs(a[i]) *n )%n;
    auto it = m.find(prefix_sum);
    if (it != m.end())
      m[prefix_sum].push_back(i);
    else
      m[prefix_sum] = {i};
  }
  prefix_sum = 0;
  ...
  range(i,n) {
    auto it = m.find(prefix_sum);
    debug(i,prefix_sum,a[i]);
    if (it != m.end()) {
      debug(i,a[i],it->second);
      auto ind = lower_bound(all(it->second),i);
      if (ind != it->second.end())
        answer += distance(ind, it->second.end());
    }
    prefix_sum = (prefix_sum + a[i] + abs(a[i]) *n)%n;
